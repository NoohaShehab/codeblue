  ...
  return (
    ...
    >
      ...
      >
        ...
        {expanded ? (
          <ChevronDown className="mt-1 h-4 w-4 shrink-0 text-[#7a918e]" />
        ) : (
          <ChevronRight className="mt-1 h-4 w-4 shrink-0 text-[#7a918e]" />
        )}
      </button>
      {expanded && (
...
export function DigitalTwin() {
  ...
  const rooms = Array.from({ length: 30 }, (_, i) => ({
    n: i + 1,
    status: i < 23 ? "occupied" : i < 26 ? "ready" : "cleaning",
  }));
  const selectedKey =
    selected === "er"
      ? "digital.emergency"
        ...
        <div className={`${card} p-5`}>
          ...
          <div className="hairline-grid rounded-xl bg-[#edf1ec] p-4 md:p-7">
            ...
            <div className="grid grid-cols-5 gap-2 md:grid-cols-6 md:gap-3">
              {rooms.map((r) => (
                <button
                  key={r.n}
                  onClick={() =>
                    setSelected(r.status === "occupied" ? "med" : "surg")
                  }
            ...
            <div className="mt-5 grid grid-cols-2 gap-2 md:grid-cols-4">
              <TwinStat value="23" label="common.occupied" />
              <TwinStat value="3" label="digital.readyNow" tone="teal" />
              <TwinStat value="4" label="common.cleaning" tone="amber" />
              <TwinStat value="0" label="common.blocked" tone="slate" />
            </div>
          </div>
        </div>
        <div className={`${card} p-5`}>
          ...
          <div className="rounded-xl bg-[#e8f3ee] p-4">
            <div className="flex items-center justify-between">
              ...
              <StatusPill status={selected === "er" ? "critical" : "watch"} />
            </div>
            <div className="mt-5 grid grid-cols-2 gap-4">
              <Metric
                label="table.occupancy"
                value={selected === "er" ? "94%" : "91%"}
                sub="digital.ofStaffedBeds"
              />
              <Metric
                label="table.available"
                value={selected === "er" ? "3" : "11"}
                sub="common.ready"
              />
            </div>
          </div>
          <div className="mt-5 space-y-4">
            <TwinRow label="digital.admissionsTransit" value="6" />
            <TwinRow label="digital.pendingTransfers" value="5" />
            <TwinRow label="digital.dischargesPlanned" value="9" />
          </div>
        </div>
      </div>
    </div>
...
function SimulationOutcome({ result }: { result: SimulationResponse }) {
  const { t } = useI18n();
    ...
    <div className="animate-rise">
      ...
      >
        <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.16em] text-[#71857f]">
          <Gauge className="h-4 w-4" /> <T id="simulation.suggestedPosture" />
        </div>
      ...
      {result.warnings.length > 0 && (
        <div className="mt-5 text-xs text-[#a37632]">
